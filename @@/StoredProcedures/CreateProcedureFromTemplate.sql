...
CREATE PROC [@@].[CreateProcedureFromTemplate]
...
BEGIN
  ...
  EXECUTE [@@].[_fillTemplate] @Template OUTPUT
  ...
  ,@LinkedServer=@LinkedServer OUTPUT
  ,@ProcedureName=@ProcedureName OUTPUT
  ,@ParameterNameList=@ParameterNameList

	DECLARE @CreateProcedureTemplate NVARCHAR (MAX)='CREATE OR ALTER PROCEDURE [@01@].[@01@]'+[@@].CRLF()
	...
	BEGIN
		...
		IF @ProcedureParameter > '' 
			SET @CreateProcedureTemplate=@CreateProcedureTemplate+'@02@'+[@@].CRLF()
	END

	DECLARE @BodyProcedureTemplate NVARCHAR (MAX)='BEGIN'+[@@].CRLF()+'@03@'+[@@].CRLF()+'END'+[@@].CRLF()
  IF [@Info].[isProcedure](@BodyProcedureTemplateName) > 0
	BEGIN
		EXECUTE [@@].[_getObjectTemplate] @BodyProcedureTemplateName,@BodyProcedureTemplate OUTPUT,@OnlyBeginEndBlock=1
	END
	SET @CreateProcedureTemplate=@CreateProcedureTemplate+'AS'+[@@].CRLF()+@BodyProcedureTemplate

  EXECUTE [@@].[_replaceTemplateParameterValue] @CreateProcedureTemplate OUTPUT,'@01@',@ProcedureName
  EXECUTE [@@].[_replaceTemplateParameterValue] @CreateProcedureTemplate OUTPUT,'@02@',@ProcedureParameter
  EXECUTE [@@].[_replaceTemplateParameterValue] @CreateProcedureTemplate OUTPUT,'@03@',@Template

	SET @Template=@CreateProcedureTemplate

  EXEC [@@].[_executeSQL] @Template,@Database,@LinkedServer,@Print=@Print
END
GO
